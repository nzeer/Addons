...
--	The Do It Now bit
	local function DoItNow(frame)
		...
		-- Minimap target blip
		if LeaPlusOptionsLC["LeaPlusTargetBlip"] == "On" then
			Minimap:SetBlipTexture("Interface\\AddOns\\Leatrix_Plus\\Leatrix_Plus.tga")
		else
...
----------------------------------------------------------------------
	...
	local function eventHandler(frame, event, arg1)
		...
		if (event == "ADDON_LOADED" and arg1 == "Leatrix_Plus") then
			...
			if LeaPlusOptionsDB["LeaPlusTargetBlip"] 		== nil then LeaPlusOptionsLC["LeaPlusTargetBlip"]	 	= "On"	else LeaPlusOptionsLC["LeaPlusTargetBlip"] 		= LeaPlusOptionsDB["LeaPlusTargetBlip"] 	end 
		...
		if (event == "PLAYER_LOGOUT") then
			...
			LeaPlusOptionsDB["LeaPlusTargetBlip"] 		= LeaPlusOptionsLC["LeaPlusTargetBlip"]
		end
...
-- 	Define the options panel
	...
	function LeaPlusOptionsPanel:OptionsPanel()
		...
		LeaPlusCheckbox("LeaPlusTargetBlip",	"Hide minimap target", 			16, -252, 	"If checked, the minimap target blip and focus blip will not be shown.")

		OptionsText("Graphics", 16, -292);
		LeaPlusCheckbox("LeaPlusDeathEffect", 	"Hide death effect", 			16, -312, 	"If checked, the death effect (grey glow) will not be shown when you are a ghost.")
		LeaPlusCheckbox("LeaPlusNetherEffect", 	"Hide netherworld", 			16, -332, 	"If checked, the netherworld effect (such as mage Invisibility) will not be shown.")
		LeaPlusCheckbox("LeaPlusGlowEffect", 	"Remove screen glow", 			16, -352, 	"If checked, the screen glow will not be shown.  Note that checking this option will reduce overall graphics quality, but you can use it when required to hide the drunken haze (blurry screen).")
		...
		LeaPlusDeflt:SetScript("OnClick", function() 
			...
			LeaPlusOptionsLC["LeaPlusTargetBlip"]		= "On"
